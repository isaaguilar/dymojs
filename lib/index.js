...
class Dymo {
  ...
  getPrinters() {
    ...
    return fetcher(`${this.apiUrl}/GetPrinters`).then(response => {
      return xmlToJSON.parseString(response.text()).Printers.forEach(types => {
        return Object.keys(types).map(i => {
          return types[i].forEach(printer => {
            return printer.Name[0]._text
          })
        })
      })
    })
